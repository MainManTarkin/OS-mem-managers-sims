...
BestFit::BestFit(int64_t intialSize)
{
...
int BestFit::allocate(int64_t allocateSize)
{
	...
	int64_t pointerOfbestBlock = headAddress;
	...
	while (true)
	{
		...
		if (freeList[headAddress].freeSpace >= allocateSize && freeList[headAddress].freeSpace <= freeList[pointerOfbestBlock].freeSpace)
		{

			pointerOfbestBlock = headAddress;
		}
	...
	if (freeList[pointerOfbestBlock].freeSpace >= allocateSize)
	{

		if (freeList[pointerOfbestBlock].nextAddress == -1)
		{

			// routine for handling last free block in list (the const last block)

			headAddress = pointerOfbestBlock;
		...
		else
		{
			...
			if (leftOverSize != 0)
			{ // other wise set up new free block from split
				headAddress += allocateSize;
				freeList[headAddress].freeSpace = leftOverSize;
	...
	cout << "Allocated: " << allocateSize << "kibibytes starting at: " << pointerOfbestBlock << endl;
	return 0;
}

int BestFit::freeBlock(int64_t indexAddress)
{
